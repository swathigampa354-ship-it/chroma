...
const { d: task } = await jf(`${BASE}/s2s/v2.0/task/hair-color`, { method: 'POST', headers: { ...AUTH, 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
console.log('task:', JSON.stringify(task).slice(0, 160))
if (!task?.data?.task_id) process.exit(1)
for (let i = 0; i < 60; i++) {
